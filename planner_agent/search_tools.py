    ...
    def fetch_many_with_jina(
        ...
        for url in urls[:limit]:
            try:
                ...
                results.append({"url": url, "content": f"Error fetching content: {str(e)}"})
    ...
    def get_top_urls(self , search_result: dict[str , Any]) -> list[str]:
        ...
        for item in search_result.get("results", []):
            ...
            if url:
                urls.append(url)
        return urls
    def get_seccessful_pages(self  , fetched_pages: list[dict[str, str]]) -> list[dict[str , str]]:
        return [page for page in fetched_pages if page.get("status") == "success" and page.get("content")]
